-- RPC rekap kumulatif STANDALONE.
-- Dibangun ulang dari awal meniru get_fasih_daily_contribution:
--   * sumber data: se2026_assignment_list + vw_fasih_wilayah_scope_base
--   * role di-derive server-side dari auth.uid()
--   * nama level & unit_id PERSIS sama dengan RPC harian, sehingga _mergeRows()
--     di dashboard menyatukan baris kumulatif & harian dengan benar dan angkanya
--     konsisten.
-- TIDAK memanggil _fasih_rekap_build / mv_fasih_assignment_summary lagi.
--
-- Perbedaan dengan harian: di sini menghitung KUMULATIF total assignment per unit
-- (semua status, termasuk OPEN) + breakdown status (status_counts). Bukan delta.
