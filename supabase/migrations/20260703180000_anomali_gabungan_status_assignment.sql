-- Tambah kolom status_assignment (dari se2026_assignment_list.status_text,
-- join by assignment_id) + tetap kirim status_tindak_lanjut yang sudah ada
-- (dipakai Dart utk warna card: sudah diperiksa = status_tindak_lanjut <>
-- 'belum_diperiksa'). Return type berubah -> perlu drop dulu sebelum
-- create ulang (CREATE OR REPLACE tidak bisa ubah kolom RETURNS TABLE).
