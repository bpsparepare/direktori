...
--
-- PERUBAHAN (revisi): assignment berstatus OPEN DIKECUALIKAN dari perhitungan
-- dashboard (total_assignment, total_terkirim, status_counts, chart). DRAFT dan
-- status lain tetap dihitung. Daftar SLS (wilayah_universe) tetap lengkap,
-- hanya angkanya yang tidak lagi menghitung OPEN.
  ...
  scoped as (
    ...
    where (v_level <> 'pendata_wilayah'  or ppl_id = v_petugas_id)
      ...
      and (v_level not in ('pengawas_wilayah','admin_wilayah')
           or ppl_id = v_petugas)
      -- Kecualikan status OPEN dari perhitungan (DRAFT & lainnya tetap dihitung).
      and upper(status_alias) not like 'OPEN%'
  ),
  normalized as (
    select
      case v_level
