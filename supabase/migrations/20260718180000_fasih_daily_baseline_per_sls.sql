...
--
-- REVISI 2 (baseline per-SLS): baseline "kemarin" diambil PER SLS (snapshot
-- terakhir SLS itu <= tanggal batas), bukan satu tanggal global. Ini menahan
-- snapshot PARSIAL: SLS yang tak ada di snapshot suatu hari otomatis memakai
-- nilai hari sebelumnya, sehingga tidak muncul lonjakan delta palsu.
--
...
begin
  ...
  -- Tanggal snapshot terbaru <= target (untuk meta).
  select max(snapshot_date) into v_snap_today
  from public.se2026_rekap_sls_harian where snapshot_date <= p_target_date;
  -- Batas atas baseline "kemarin" = snapshot terbaru <= (target-1).
  select max(snapshot_date) into v_yest_bound
  from public.se2026_rekap_sls_harian where snapshot_date <= v_yesterday;
  -- Tak ada snapshot sebelum hari ini (mis. impor pertama) → baseline = hari ini
  -- (delta 0), hindari lonjakan palsu.
  if v_yest_bound is null then v_yest_bound := p_target_date; end if;

  with
  -- Snapshot TERBARU per SLS pada/atau sebelum tanggal acuan. Memakai per-SLS
  -- (bukan satu tanggal global) agar TAHAN snapshot parsial: SLS yang absen di
  -- suatu tanggal otomatis memakai nilai tanggal sebelumnya.
  rt as (
    select distinct on (level_6_full_code) *
    from public.se2026_rekap_sls_harian
    where snapshot_date <= p_target_date
    order by level_6_full_code, snapshot_date desc
  ),
  ry as (
    select distinct on (level_6_full_code) *
    from public.se2026_rekap_sls_harian
    where snapshot_date <= v_yest_bound
    order by level_6_full_code, snapshot_date desc
  ),
  scope_base as (
    select
    ...
    left join rt on rt.level_6_full_code = sw.kode_wilayah
    left join ry on ry.level_6_full_code = sw.kode_wilayah
  ),
  scoped as (
    select *
    from scope_base
