-- Satukan sumber "perlu ditindaklanjuti":
--  1) Hapus get_anomali_konfirmasi_count -- digantikan get_anomali_pusat_notif_count
--     (satu-satunya penghitung, lihat 20260708180000).
--  2) get_anomali_gabungan + kolom per-baris `perlu_tindak_lanjut` dengan
--     definisi PERSIS sama dengan penghitung badge, supaya tanda di kartu dan
--     angka di tab selalu konsisten:
--       pendata/pengawas -> belum saya respons (r_mine.jenis_respons null).
--       admin            -> sudah dijawab petugas (perbaikan/konfirmasi_valid)
--                           tapi belum ada keputusan verifikasi (vf.status null).
--
-- Sisanya identik dengan 20260708170000_anomali_pusat_verifikasi_reject.sql.

drop function if exists public.get_anomali_konfirmasi_count();
