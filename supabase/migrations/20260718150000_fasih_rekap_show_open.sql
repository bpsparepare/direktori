...
--
-- REVISI: OPEN tidak dihitung ke Total/Terkirim, TAPI tetap DITAMPILKAN di
-- rincian status (supaya lengkap). OPEN diturunkan = total - Σ status bernama
-- (OPEN tak punya kolom di rekap).
--   total_assignment (Total Kumulatif) = Σ status bernama (termasuk DRAFT, TANPA OPEN)
--   total_terkirim   (Terkirim/final)  = Σ status bernama tanpa DRAFT & tanpa OPEN
--   status_counts                      = pecahan per status + baris 'OPEN' (info)
-- Catatan: karena OPEN tampil di rincian tapi tak masuk total, jumlah seluruh
-- baris status bisa > Total Kumulatif (selisihnya = OPEN). Ini disengaja.
...
begin
  ...
  scope_base as (
    select
      ...
      -- OPEN diturunkan: total (COUNT, memuat OPEN) - Σ status bernama.
      -- Hanya untuk DITAMPILKAN di rincian; TIDAK masuk m_total/m_terkirim.
      greatest(coalesce(r.total,0)
        - (coalesce(r.approved_pengawas,0) + coalesce(r.submitted_pencacah,0)
           + coalesce(r.submitted_respondent,0) + coalesce(r.draft_awal,0)
           + coalesce(r.draft_revisi,0) + coalesce(r.rejected_pengawas,0)
           + coalesce(r.edited_admin_kab,0) + coalesce(r.rejected_admin_kab,0)
           + coalesce(r.revoked_pengawas,0)), 0)                          as m_open
