-- Perbaikan penentuan kategori_kode saat import anomali pusat.
--
-- Masalah: import_anomali_pusat_batch sebelumnya menyimpan kategori_kode
-- sebagai angka mentah saja ("1", "3", ...) atau "lainnya", sehingga usaha
-- dan keluarga bisa bertabrakan kode dan sulit dibedakan.
--
-- Perbaikan: kode diberi prefix per scope -> 'usaha' = UP, 'keluarga' = KP.
--   "Anomali Data 3 (Biaya Produksi Dominan)" + usaha    -> UP3
--   "Anomali Data 3 (...)"                    + keluarga -> KP3
--   tidak cocok pola                          + usaha    -> UPLAINNYA
-- Perilaku p_mode (refresh/replace/tambahkan) dan kolom dihapus tetap sama.
--
-- Catatan: tabel public.anomali_pusat_kategori diasumsikan sudah ada.
-- Fungsi ini meng-upsert kode berprefix (UP*/KP*) ke tabel tersebut.
