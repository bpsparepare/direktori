-- Verifikasi anomali pusat menjadi KHUSUS ADMIN:
-- - Hanya admin yang boleh set/batal verifikasi.
-- - Selain admin tidak melihat status verifikasi (kolom di-null-kan) maupun
--   tombolnya (boleh_verifikasi = false).
...
begin
    ...
    if v_role <> 'admin' then
        raise exception 'Hanya admin yang dapat memverifikasi';
    end if;
...
-- get_anomali_gabungan: verified_at/verified_oleh/boleh_verifikasi hanya untuk
-- admin (signature & kolom tetap sama, jadi cukup create or replace).
...
begin
    ...
    v_verif_at := case
        when v_role = 'admin' then 'vf.updated_at'
        else 'null::timestamptz'
    end;
    v_verif_by := case
        when v_role = 'admin' then 'vp.nama'
        else 'null::text'
    end;
    v_can_verify := case
