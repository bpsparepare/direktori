...
--
-- "terkirim" = total - draft_awal - draft_revisi. Kontrak JSON & unit_id per
-- level identik dgn versi lama → merge dengan get_fasih_rekap tetap benar.
-- p_progress_mode dipertahankan untuk kompatibilitas tetapi tidak dipakai.
...
begin
  ...
  scope_base as (
    select
      ...
      greatest(coalesce(rt.total,0) - coalesce(rt.draft_awal,0)
               - coalesce(rt.draft_revisi,0), 0) as terk_today,
      greatest(coalesce(ry.total,0) - coalesce(ry.draft_awal,0)
               - coalesce(ry.draft_revisi,0), 0) as terk_yest,
