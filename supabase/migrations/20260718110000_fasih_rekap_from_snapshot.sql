...
--
-- Pemetaan angka:
--   total_assignment (Total Kumulatif) = total
--   total_terkirim   (Terkirim/final)  = total - draft_awal - draft_revisi
--   status_counts                      = pecahan per status (kolom apa adanya)
...
begin
  ...
  scope_base as (
    select
      ...
      coalesce(r.total, 0)                                                as m_total,
      greatest(coalesce(r.total,0)
               - coalesce(r.draft_awal,0)
               - coalesce(r.draft_revisi,0), 0)                          as m_terkirim,
