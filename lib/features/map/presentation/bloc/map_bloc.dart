...
class MapBloc extends Bloc<MapEvent, MapState> {
  ...
  MapBloc({required this.getInitialMapConfig, required this.getPlaces, required this.getFirstPolygonMeta, required this.getAllPolygonsMeta})
  ...
  Future<void> _onPlacesRequested(
      PlacesRequested event, Emitter<MapState> emit) async {
    try {
  ...
  Future<void> _onPolygonRequested(
      PolygonRequested event, Emitter<MapState> emit) async {
    try {
      final meta = await getFirstPolygonMeta(event.assetPath);
      debugPrint('BLoC: polygon points loaded = ${meta.points.length}, label = ${meta.name}');
      emit(state.copyWith(polygon: meta.points, polygonLabel: meta.name, selectedPolygonMeta: meta));
    } catch (e) {
  ...
  Future<void> _onPolygonsListRequested(
      PolygonsListRequested event, Emitter<MapState> emit) async {
    try {
      ...
      PolygonData? selMeta = state.selectedPolygonMeta;
      if (selPoints.isEmpty && list.isNotEmpty) {
        selPoints = list.first.points;
        selLabel = list.first.name;
        selMeta = list.first;
      }
      emit(state.copyWith(polygonsMeta: list, polygon: selPoints, polygonLabel: selLabel, selectedPolygonMeta: selMeta));
    } catch (e) {
  ...
  void _onPolygonSelectedByIndex(PolygonSelectedByIndex event, Emitter<MapState> emit) {
    if (event.index < 0 || event.index >= state.polygonsMeta.length) return;
    final sel = state.polygonsMeta[event.index];
    emit(state.copyWith(polygon: sel.points, polygonLabel: sel.name, selectedPolygonMeta: sel));
  }
