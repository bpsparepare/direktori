...
class ContributionRepositoryImpl implements ContributionRepository {
  ...
  Future<UserContribution> createContribution({
    ...
    try {
      print('🔄 [CONTRIBUTION] Membuat kontribusi baru...');
      print('👤 [CONTRIBUTION] User ID: $userId');
      print('🎯 [CONTRIBUTION] Action Type: $actionType');
      print('📍 [CONTRIBUTION] Target Type: $targetType');
      print('🆔 [CONTRIBUTION] Target ID: $targetId');
      print('📊 [CONTRIBUTION] Changes: $changes');
      print('🌍 [CONTRIBUTION] Koordinat: $latitude, $longitude');

      // Sisipkan action_subtype ke dalam changes hanya jika belum ada
      final enrichedChanges = {...(changes ?? {})};
      if (!enrichedChanges.containsKey('action_subtype')) {
        enrichedChanges['action_subtype'] = actionType;
      }
      ...
      print('💾 [CONTRIBUTION] Menyimpan ke database...');
      final result = await _remoteDataSource.createContribution(contribution);
      print(
        '✅ [CONTRIBUTION] Kontribusi berhasil disimpan dengan ID: ${result.id}',
      );
      return result.toEntity();
    } catch (e) {
      print('❌ [CONTRIBUTION] Gagal membuat kontribusi: $e');
  ...
  Future<List<UserContribution>> getUserContributions({
    ...
    try {
      print('🔍 [CONTRIBUTION] Mengambil kontribusi user...');
      print('👤 [CONTRIBUTION] User ID: $userId');
      print(
        '📊 [CONTRIBUTION] Limit: $limit, Offset: $offset, Status: $status',
      );
      ...
      print(
        '📋 [CONTRIBUTION] Ditemukan ${result.length} kontribusi dari database',
      );
      ...
      if (status != null) {
        final filtered = result
            ...
            .toList();
        print(
          '🔍 [CONTRIBUTION] Setelah filter status "$status": ${filtered.length} kontribusi',
        );
        return filtered;
      }
      ...
      print(
        '✅ [CONTRIBUTION] Berhasil mengambil ${contributions.length} kontribusi',
      );
      return contributions;
    } catch (e) {
      print('❌ [CONTRIBUTION] Gagal mengambil kontribusi user: $e');
