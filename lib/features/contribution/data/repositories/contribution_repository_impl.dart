...
class ContributionRepositoryImpl implements ContributionRepository {
  ...
  Future<UserContribution> createContribution({
    ...
    try {
      ...
      final contribution = UserContributionModel(
        ...
        targetId: targetId,
        changes: changes,
        points: _calculatePoints(actionType), // Helper method to calculate points
        status: 'pending',
        latitude: latitude,
        longitude: longitude,
      ...
      print('✅ [CONTRIBUTION] Kontribusi berhasil disimpan dengan ID: ${result.id}');
  ...
  Future<List<UserContribution>> getUserContributions({
    ...
    try {
      ...
      print('👤 [CONTRIBUTION] User ID: $userId');
      print('📊 [CONTRIBUTION] Limit: $limit, Offset: $offset, Status: $status');
      ...
      print('📋 [CONTRIBUTION] Ditemukan ${result.length} kontribusi dari database');
      ...
      if (status != null) {
        final filtered = result
            ...
            .toList();
        print('🔍 [CONTRIBUTION] Setelah filter status "$status": ${filtered.length} kontribusi');
        return filtered;
      }
      
      final contributions = result.map((model) => model.toEntity()).toList();
      print('✅ [CONTRIBUTION] Berhasil mengambil ${contributions.length} kontribusi');
  ...
  bool get isUserLoggedIn {
    if (_remoteDataSource is ContributionRemoteDataSourceImpl) {
      return (_remoteDataSource as ContributionRemoteDataSourceImpl).isUserLoggedIn;
    }
    return false;
  }
}